...
namespace ft
{
	...
	private : 
		...
		// private function for Modifiers
		void	__reallocate(size_type n);
		void	__reconstruct_push_back(const value_type& val);

		// object ready to increase size 
		// takes a vector object and size 
	...
	{
		ConstructTransaction tx(*this, n);
		for (; tx.pos_ != tx.new_end_ && first != last ; ++first)
			this->alloc().construct(tx.pos_++, *first);
	}

	// private function for push_back
	template <typename Tp, typename Allocator>
	...
	// swap old vector(this) to new vector(tmp)
	template <typename Tp, typename Allocator>
	...
	// after __swap_data, tmp(old vector) automically disappears.
	template <typename Tp, typename Allocator>
