...
/*   Updated: 2022/07/11 10:57:53 by hjeong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_check(t_mlxdata *data)
{
	...
	{
		wid = 0;
		if (ft_strlen(data->map[hei]) != data->width)
			error_print("map format error\n");
		while (wid < data->width)
		{
			if (data->map[hei][wid] == 'P')
			{
				data->loc.x = wid;
				data->loc.y = hei;
			}
...
void	map_init(char *argv, t_mlxdata *data)
{
	...
	if (ft_strnstr(argv, ".ber", ft_strlen(argv)) != &argv[ft_strlen(argv) - 4])
